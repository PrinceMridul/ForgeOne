...
const IRREGULAR_SINGULARS: Record<string, string> = {
  ...
  // A single-syllable word ending in a lone `z` doubles it before `-es`, so
  // "quizzes" singularises to "quiz", not to "quizz". Without this entry a
  // prompt naming quizzes yielded two near-identical resources — `quizes` from
  // the domain profile's "quiz" and `quizzes` from the prompt's "quizz" — and
  // the generated repository carried a route module for each.
  quizzes: 'quiz',
};
